...
class NoticiasController {
    ...
    static async cadastraNoticia(req, res) {
        ...
        try {
            ...
            throw new Error('Houve um erro ao cadastrar a notícias! '+error);
    ...
    static async editarNoticias(req, res) {
        ...
        const objEdicaoNoticia = req.body;
		
		const novoLocalImagem = arrayEdicaoImagem[0]
                                    .length === 0 || arrayEdicaoImagem[0] === undefined? 
                                        `${objEdicaoNoticia.imagemNoticia}`
                                        :`/img/noticias/${arrayEdicaoImagem[0][0].filename}`;


        const novoLocalArquivo = arrayEdicaoArquivo[0]
                                    .length === 0 || arrayEdicaoArquivo[0] === undefined? 
                                        `${objEdicaoNoticia.arquivoPortugues}`
                                        :`http://192.168.4.121/compartilhamento/procedimentos/noticias/${arrayEdicaoArquivo[0][0].filename}`;

        const novoLocalArquivoEng = arrayEdicaoArquivoEng[0]
                                        .length === 0 || arrayEdicaoArquivoEng[0] === undefined? 
